...
var showEvents = async function showEvents(someID) {
    ...
    for (let i = 0; i < events.length; i++) {
        ...
        if (event.event === "Kick") {
            ...
            values += "lot: " + lot.toFixed(2) + " mcr | ";

            let tab = event.returnValues.bid / 10 ** 18;
            values += "bid: " + tab.toFixed(4) + " mdt | ";
            ...
            if (medianizerPrice > 0) {
                values += "Monolithos MED Price: ₽" + auctions[flapId]["kickPrice"] + " | ";
            } else {
                values += "Monolithos MED Price: ₽---,-- | ";
            }
            ...
            if (!medianizerPrice) {
                values += "Took Rate: ₽" + auctions[flapId]["paidPrice"] + " mcr/mdt (+-.--%) | ";
                values += "--,-- % | Price: ₽---,-- | ";
            } else {
                values += "Took Rate: ₽" + auctions[flapId]["paidPrice"] + " mcr/mdt ";
...
function showAuctionDetails(id) {
    ...
    msg += `- LOT: ${auction.kickLot.toLocaleString('en')} mcr - BID: ${auction.bid.toLocaleString('en')} mdt <br/>`;
    ...
    msg += `- LOT: ${auction.lot.toLocaleString('en')} mcr - BID: ${auction.bid.toLocaleString('en')} mdt <br/>`;
    msg += `- TOOK RATE: ₽${auction.paidPrice} mdt/mcr <br/>`;
    ...
    msg += `- PROFIT: <b>${daiProfit.toLocaleString('en')} mcr</b>`;
